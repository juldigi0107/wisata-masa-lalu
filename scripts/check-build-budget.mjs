...
const runtimeSvg=files.filter(file=>extname(file.path).toLowerCase()==='.svg');
const pwaIcons=files.filter(file=>file.path.includes(`${join('assets','icons')}`)&&/wml-(?:64|180|192|512|maskable-512)\.png$/.test(file.path));
const failures=[];
...
if(runtimeSvg.length)failures.push(`SVG leaked into public production artifact: ${runtimeSvg.map(file=>relative(root,file.path)).join(', ')}`);
if(pwaIcons.length!==5)failures.push(`Expected 5 generated PNG install icons, found ${pwaIcons.length}`);
...
console.log(`- PWA PNG icons: ${pwaIcons.length}/5`);
console.log(`- Public SVG artifacts: ${runtimeSvg.length}`);
