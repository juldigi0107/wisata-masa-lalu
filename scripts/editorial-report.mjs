...
const priority=audit.priorityQueue.slice(0,10);
...
const lines=[
 ...
 `- Needs attention: **${audit.needsAttention}**`,
 `- Verified entries with one or more editorial gaps: **${audit.verifiedWithIssues}**`,
 '',
 '> Completeness is a documentation-readiness metric, not a historical truth score.',
 '',
 '## Readiness',
 ...
 '| # | Entry | Type | Status | Score | Readiness | Main gaps |',
 '| ---: | --- | --- | --- | ---: | --- | --- |',
 ...priority.map((item,index)=>`| ${index+1} | ${item.title.replaceAll('|','/')} | ${item.type} | ${item.status} | ${item.completenessScore} | ${item.readiness} | ${(item.issues.slice(0,3).join(', ')||'none').replaceAll('|','/')} |`),
 ''
];
