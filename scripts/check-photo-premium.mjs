...
const primaryPageIds=['intro','onboarding','time-machine','search','collection','campaign','settings','social','contextual','moderation','recovery'];
const secondaryPageIds=['season','culture','memory-card','archive-loading','memory-wall'];
const allPageIds=[...primaryPageIds,...secondaryPageIds];
const sceneIds=['rumah','kampung','sekolah','kota','digital'];
const required=[...allPageIds.map(id=>`assets/generated/${id}-premium.webp`),...sceneIds.map(id=>`assets/world/raster/${id}-90.webp`),'assets/generated/premium-photo-manifest.json','assets/world/raster/portal-grid.webp','assets/world/raster/brand-orbit.webp','assets/cassette-player.webp','assets/handheld-game.webp','assets/ramadan-lantern.webp'];
...
if(!Array.isArray(manifest.pages)||manifest.pages.length!==primaryPageIds.length)throw new Error(`Primary premium page backdrops ${manifest.pages?.length||0}/${primaryPageIds.length}`);
...
if(total>15*1024*1024)throw new Error(`Photo-premium layer exceeds 15 MB: ${(total/1024/1024).toFixed(2)} MB`);
console.log(`Photo-premium layer: 5/5 scenes · ${allPageIds.length}/${allPageIds.length} page backdrops · ${(total/1024/1024).toFixed(2)} MB checked.`);
