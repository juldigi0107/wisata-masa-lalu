...
test("assembled catalog entries, sources, station coverage and schedule references are valid",()=>{
 ...
 assert.ok(catalog.entries.length>=39,"assembled v2.4 catalog unexpectedly small");
 ...
 assert.ok(catalog.entries.filter(e=>e.type==="bacaan").length>=3,"reading-culture batch missing");
...
test("search supports query, type, station, status and region filters",async()=>{
 ...
 const elex=await(await call("/api/entries?q=elex&type=bacaan")).json();assert.ok(elex.total>=1);assert.ok(elex.entries.some(e=>e.id==="elex-manga-90an"));
...
test("entries endpoint paginates safely for a 500+ entry catalog",async()=>{
 ...
 const reading=await(await call("/api/entries?type=bacaan&limit=2&offset=0")).json();assert.equal(reading.entries.length,2);assert.ok(reading.total>=3);assert.equal(reading.nextOffset,2);
...
test("archive schedules, source ledger, stats and facets are exposed",async()=>{
 ...
 const sources=await(await call("/api/sources")).json();assert.ok(sources.total>=35);assert.equal(sources.total,sources.sources.length);assert.equal(new Set(sources.sources.map(s=>s.id)).size,sources.total);
 const stats=await(await call("/api/stats")).json();assert.equal(stats.version,catalog.version);assert.equal(stats.total,catalog.entries.length);assert.ok(stats.byType.mainan>=9);assert.ok(stats.byType.kartun>=5);assert.ok(stats.byType.musik>=6);assert.ok(stats.byType.bacaan>=3);assert.ok(stats.byType.jajanan>=3);assert.ok(stats.byStatus.verified>=1);
 const facets=await(await call("/api/facets")).json();assert.equal(facets.version,catalog.version);assert.equal(facets.total,catalog.entries.length);assert.equal(facets.byType.mainan,stats.byType.mainan);assert.equal(facets.byType.bacaan,stats.byType.bacaan);assert.ok(facets.byStation.RCTI>=1);assert.ok(Object.keys(facets.byRegion).length>=1);
});
