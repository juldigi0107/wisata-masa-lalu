...
/* Production visuals are generated raster assets. Editable vector files are authoring-only
   and scripts/scrub-runtime-svg.mjs removes them from the public artifact. */
...
const contextualStates={rumah:['ramadan','minggu','malam-minggu'],kampung:['hujan','ramadan','agustusan','minggu'],sekolah:['agustusan'],kota:['ramadan','malam-minggu'],digital:[]};
...
try{
 ...
 if(stored){
  ...
  stored.completed=completed;stored.score=completed.reduce((sum,id)=>sum+(getTrigger(id)?.points||0),0);stored.schema=4;
  if(hasScene){stored.scene=requestedScene;stored.visitedScenes=[...new Set([...(stored.visitedScenes||[]),requestedScene])];}
  if(hasYear){stored.year=requestedYear;stored.visitedYears=[...new Set([...(stored.visitedYears||[]),requestedYear])];}
...
createRoot(document.getElementById("root")).render(<React.StrictMode><AppErrorBoundary><WorldExperienceShell/><AmbientRuntimeV4/><PremiumRuntimeV5/><EasterEggRuntime/></AppErrorBoundary></React.StrictMode>);
