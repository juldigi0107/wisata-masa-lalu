...
import {deepObjectCabinet} from './deepDive.js';
import DeepDiveSections from './DeepDiveSections.jsx';
import {ArchiveFilterTools,QuizResultActions,StoryPager} from './ArchiveFeatureExtras.jsx';
import './deepDive.css';

const BASE=import.meta.env.BASE_URL||'/';
const ASSET=`${BASE}assets/`;
const MEDIA=`${ASSET}media/`;
const ALL_OBJECTS=[...objectCabinet,...deepObjectCabinet];
...
export default function ArchiveAppFull(){
 ...
 useEffect(()=>{
  ...
  fetch(base.replace(/\/$/,'')+'/api/catalog',{signal:controller.signal})
   ...
   .then(d=>{
    ...
    if(stopped)return;
    setData(d);setActive(d.entries[0]);
 ...
 useEffect(()=>{setVisibleCount(12)},[query,typeFilter,statusFilter]);
 ...
 async function click(force=false){
  ...
  try{
   ...
   o.type='square';o.frequency.setValueAtTime(430,c.currentTime);o.frequency.exponentialRampToValueAtTime(95,c.currentTime+.08);
   g.gain.setValueAtTime(.02,c.currentTime);g.gain.exponentialRampToValueAtTime(.001,c.currentTime+.09);
   o.connect(g);g.connect(c.destination);o.start();o.stop(c.currentTime+.1);o.onended=()=>{o.disconnect();g.disconnect()};
  }catch{setSoundError('Efek suara tidak tersedia pada perangkat ini.')}
 }
 function openEntry(entry){if(!entry)return;setActive(entry);requestAnimationFrame(()=>detail.current?.focus())}
 function answer(i){
  const q=quizQuestions[quizStep];const next=quizScore+(i===q[2]?1:0);setQuizScore(next);
  if(quizStep===quizQuestions.length-1)setQuizDone(true);else setQuizStep(quizStep+1);
 }
 function resetQuiz(){setQuizStep(0);setQuizScore(0);setQuizDone(false)}
 function resetFilters(){setQuery('');setTypeFilter('semua');setStatusFilter('semua');setVisibleCount(12)}
 ...
 const statusStats=useMemo(()=>data.entries.reduce((acc,e)=>{acc[e.status]=(acc[e.status]||0)+1;return acc},{}),[data.entries]);
 const sourceCount=useMemo(()=>new Set(data.entries.flatMap(e=>(e.sources||[]).map(s=>s.id))).size,[data.entries]);
 const filtered=useMemo(()=>data.entries.filter(e=>{
  ...
  const haystack=`${e.title} ${(e.tags||[]).join(' ')} ${e.summary} ${e.details?.station||''} ${e.details?.region||''} ${e.details?.genre||''}`.toLocaleLowerCase('id');
  return matchesType&&matchesStatus&&haystack.includes(needle);
 }),[data.entries,query,typeFilter,statusFilter]);
 useEffect(()=>{if(filtered.length&&!filtered.some(entry=>entry.id===active?.id))setActive(filtered[0])},[filtered,active?.id]);
 ...
 const object=ALL_OBJECTS[objectIndex]||ALL_OBJECTS[0];
 const archive=data.archiveSchedules?.[0];
 const verifiedCount=statusStats.verified||0;
 ...
 return <div className={night?'app-shell night-ready':'app-shell'}>
  ...
  <header className="site-header"><div className="header-inner">
   ...
   <nav aria-label="Navigasi utama"><a href="#tv">TV</a><a href="#games">Kampung</a><a href="#rental">Benda</a><a href="#timeline">Dekade</a><a href="#warung">Warung</a><a href="#collection">Arsip</a></nav>
  ...
  <main id="main">
   <section id="top" className="hero editorial-grid">
    ...
    <div className="hero-copy"><Eyebrow>ISSUE 05 · 1990—1999 · INDONESIA</Eyebrow><h1>Yang kita rindukan ternyata <em>bukan cuma zamannya.</em></h1><p className="hero-lead">Masuk lewat suara TV ruang tengah, uang receh di saku seragam, permainan sore, komik dan majalah yang berpindah tangan, alat tulis favorit, pager yang berbunyi tiba-tiba, sampai malam Ramadan ketika jalan kampung belum cepat sepi.</p><div className="hero-actions"><a className="primary" href="#index">Buka ensiklopedia</a><a className="text-link" href="#timeline">Lihat kronologi dekade ↘</a></div><div className="hero-meta"><span><b>{data.entries.length}</b> entri kurasi</span><span><b>{verifiedCount}</b> terverifikasi</span><span><b>{sourceCount}</b> sumber unik</span><span><b>{assetCredits.length}</b> aset terlacak</span></div></div>
   ...
   <section id="index" className="index-section wrap"><div className="section-head"><div><Eyebrow>DAFTAR ISI / MEMORY INDEX</Eyebrow><h2>Delapan pintu masuk.</h2></div><p>Setiap bagian memakai bahasa visual berbeda—majalah, katalog warung, blueprint, layar CRT, mixtape, timeline, dan data map—tetapi semuanya kembali ke SSOT dan provenance yang dapat dilacak.</p></div><div className="module-grid">{modules.map(m=><a key={m.id} className={`module-card ${m.tone}`} href={`#${m.id}`}><span className="module-no">{m.no}</span><span className="module-icon">{m.icon}</span><h3>{m.title}</h3><p>{m.subtitle}</p><span className="module-arrow">↘</span></a>)}</div></section>

   <section id="tv" className="tv-section"><div className="wrap"><div className="section-head light"><div><Eyebrow>01 / VIRTUAL CRT</Eyebrow><h2>Remote-nya di bawah bantal.</h2></div><p>Enam kanal mempunyai entri kurasi. Layar ini simulasi editorial, bukan streaming ulang siaran berhak cipta.</p></div><div className="tv-layout">
   ...
   <section id="rental" className="object-section"><div className="wrap"><div className="section-head"><div><Eyebrow>04 / RENTAL & OBJECT CABINET</Eyebrow><h2>Benda kecil, memori besar.</h2></div><p>Kabinet sekarang memuat {ALL_OBJECTS.length} object-study: game, audio, media simpan, komunikasi, sekolah, dan kultur remaja. Visual berhak cipta seperti poster film tetap tidak disalin.</p></div><div className="object-layout"><div className="object-stage"><Picture src={`${MEDIA}${object.image}`} alt={`${object.name}, objek studi nostalgia`}/><span>{object.kicker} · {object.year}</span></div><div className="object-copy"><Eyebrow>OBJECT {String(objectIndex+1).padStart(2,'0')} / {String(ALL_OBJECTS.length).padStart(2,'0')}</Eyebrow><h2>{object.name}</h2><p>{object.story}</p><a className="source-link dark" href={object.source} target="_blank" rel="noreferrer">{object.sourceLabel} ↗</a>{object.entryId&&<button className="source-link dark" onClick={()=>openEntry(data.entries.find(e=>e.id===object.entryId))}>Buka entri SSOT ↘</button>}<div className="object-nav">{ALL_OBJECTS.map((o,i)=><button key={o.id} aria-label={`Pilih ${o.name}`} aria-pressed={i===objectIndex} onClick={()=>setObjectIndex(i)}><span>{String(i+1).padStart(2,'0')}</span>{o.name}</button>)}</div></div></div></div></section>

   <DeepDiveSections data={data} onOpenEntry={openEntry}/>
   ...
   <section id="collection" className="collection wrap"><div className="section-head"><div><Eyebrow>ARSIP / KLIPING</Eyebrow><h2>Buka laci kenangan.</h2></div><label>Cari judul, kanal, daerah, atau topik<input type="search" placeholder="Coba: HAI, pager, Nintendo, RCTI, Bobo" value={query} onChange={e=>setQuery(e.target.value)}/></label></div>
    ...
    <div className="status-filter" role="group" aria-label="Filter status provenance"><span>Provenance</span><button aria-pressed={statusFilter==='semua'} onClick={()=>setStatusFilter('semua')}>Semua</button>{['verified','curated'].filter(status=>statusStats[status]).map(status=><button key={status} aria-pressed={statusFilter===status} onClick={()=>setStatusFilter(status)}>{status} · {statusStats[status]}</button>)}</div>
    <ArchiveFilterTools query={query} typeFilter={typeFilter} statusFilter={statusFilter} onReset={resetFilters}/>
    ...
    <div className="clip-grid">{visibleEntries.map((e,i)=><button className={`clip-card c${i%4}`} key={e.id} onClick={()=>openEntry(e)}><span className="clip-status">{e.status}</span><span>{String(i+1).padStart(2,'0')}</span><small>{e.type} · {e.details?.station||e.details?.region||''}</small><h3>{e.title}</h3><p>{e.summary}</p><b>Buka kliping ↗</b></button>)}</div>
    ...
    {!filtered.length&&<p className="empty-state">Belum ada entri yang cocok dengan kombinasi pencarian, kategori, dan provenance itu.</p>}
    {filtered.length>0&&<><StoryPager entries={filtered} active={active} onOpen={openEntry}/><article ref={detail} tabIndex="-1" className="feature-story"><div className="story-main"><Eyebrow>FOCUS STORY / {active.type?.toUpperCase()}</Eyebrow><h2>{active.title}</h2>{activeVisual&&<figure className="story-visual"><Picture src={entryAssetSrc(activeVisual)} alt={activeVisual.alt||`Object study ${active.title}`}/><figcaption><span>OBJECT STUDY</span>{activeVisual.credit||'Visual berlisensi · lihat ledger aset'}</figcaption></figure>}<div className="meta-chips"><span>{active.status==='verified'?'✓ VERIFIED':`◌ ${String(active.status||'curated').toUpperCase()}`}</span>{active.details?.station&&<span>{active.details.station}</span>}{active.details?.region&&<span>{active.details.region}</span>}{active.details?.premiere&&<span>{active.details.premiere}</span>}{active.details?.genre&&<span>{active.details.genre}</span>}</div><div className="story-source-summary"><span>{activeSources.length} sumber entri</span><span>{active.factBox?.sourceIds?.length||0} sumber Fact Box</span><span>{active.quoteBox?.kind==='editorial-fiction'?'Quote editorial':'Quote bersumber'}</span></div><p className="dropcap">{active.summary}</p>{active.details?.context&&<p>{active.details.context}</p>}{active.details?.people&&<p className="people"><b>Tokoh/kredit terpilih:</b> {active.details.people}</p>}<div className="story-sources"><Eyebrow>SUMBER RISET</Eyebrow>{activeSources.length?activeSources.map(s=><a key={s.id} href={s.url} target="_blank" rel="noreferrer"><b>{s.title}</b><small>{s.kind} · dicek {s.checkedAt}</small></a>):<p>Belum ada sumber eksternal pada entri ini; statusnya tidak boleh dianggap verified.</p>}</div></div><div className="story-notes"><section><span>FUN FACT</span><p>{active.factBox?.text}</p></section><section><span>SUARA NOSTALGIA</span><blockquote>“{active.quoteBox?.text}”</blockquote><small>{active.quoteBox?.attribution}</small></section><section><span>PRICE TAG</span><p>{active.priceTag?.note}</p></section></div></article></>}
   </section>

   <section className="credits wrap"><div><Eyebrow>VISUAL SOURCE LEDGER</Eyebrow><h2>Aset internet tetap punya nama.</h2><p>Foto diunduh saat build lalu disimpan lokal pada artifact Pages. Aset inti mempunyai quality gate; object-study detail memakai graceful fallback bila sumber eksternal sedang throttled. Kredit tetap ditampilkan walaupun file visual sementara tidak tersedia.</p></div><div className="credit-list">{assetCredits.map(c=><a key={c.file} href={c.url} target="_blank" rel="noreferrer"><b>{c.label}</b><span>{c.author} · {c.license}</span></a>)}</div></section>
  </main>
