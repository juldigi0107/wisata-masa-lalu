...
export default function WorldAppV4(){
 ...
 const phase=useClockPhase();const isOnline=useOnlineState();const audio=useSynthAudio(settings);const scene=getScene(sceneId);const daily=memoryTriggers[Math.floor(dailyRandom()*memoryTriggers.length)];const completed=profile?.completed||[];const owned=profile?.collections||[];const visitedScenes=profile?.visitedScenes||[sceneId];const visitedYears=profile?.visitedYears||[year];const score=profile?.score||0;
 ...
 useEffect(()=>{if(!profile||overlay||activeObject||event)return;const timer=setInterval(()=>{if(document.visibilityState==='visible'&&Math.random()<.36){const next=weightedRandomEvent(sceneId);setEvent(next);showToast(next.label,4200)}},26000);return()=>clearInterval(timer)},[profile,sceneId,overlay,activeObject,event]);
 ...
 return <div className={`world-app intensity-${settings.intensity}`} style={{'--year-accent':yearWorldState[year].accent}}>
  ...
  {overlay==='collection'&&<CollectionPanel owned={owned} unlockedAchievements={unlockedAchievements} onClose={()=>setOverlay(null)}/>} 
  ...
  {event&&!overlay&&!activeObject&&<div className="ambient-event" role="dialog" aria-label="Kejadian random"><small>KEJADIAN RANDOM</small><b>{event.label}</b><button onClick={()=>{const item=getTrigger(event.trigger);setEvent(null);if(item)launchTrigger(item)}}>Ikuti ↘</button><button onClick={()=>setEvent(null)}>Lewati</button></div>}
 </div>;
}
