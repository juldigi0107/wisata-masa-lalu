...
import ArchivePortal from './ArchivePortal.jsx';
...
function useSynthAudio(settings){
 ...
 async function ctx(){if(settings.mute||settings.master<=0)return null;const AC=window.AudioContext||window.webkitAudioContext;if(!AC)return null;try{ctxRef.current??=new AC();await ctxRef.current.resume();return ctxRef.current}catch{return null}}
...
function Intro({onDone}){
 ...
 return <div className="time-intro" role="dialog" aria-modal="true" aria-label="Mesin waktu"><div className="intro-noise"/><p>WISATA MASA LALU / TIME MACHINE</p><div className="intro-year" key={step} aria-live="polite">{labels[step]}</div><span>{step<4?'Kalender bergerak mundur…':'Indonesia, tahun 1990-an.'}</span>{step>=4&&<button onClick={onDone}>MASUK KE 90-AN ↘</button>}<button className="intro-skip" autoFocus onClick={onDone}>Lewati intro</button></div>;
}
function Onboarding({onChoose}){return <main className="onboarding"><div className="onboarding-paper"><p className="world-eyebrow">SATU PERTANYAAN SAJA</p><h1>Kamu paling ingat<br/>masa 90-an sebagai…</h1><p className="onboarding-hint">Tidak ada tutorial panjang. Pilih yang paling terasa familiar. Nanti cukup sentuh benda yang kamu kenal.</p><div className="profile-choices">{onboardingChoices.map((choice,index)=><button autoFocus={index===0} key={choice.id} onClick={()=>onChoose(choice.id)}>{choice.label}<span>↘</span></button>)}</div></div></main>}
...
export default function WorldAppV4(){
 ...
 useEffect(()=>{completionLocks.current=new Set(completed)},[completed]);
 ...
 function chooseProfile(type){const choice=nostalgiaProfiles.find(item=>item.id===type)||nostalgiaProfiles[0];const next=normalizeProfile({type:choice.id,scene:choice.scene,year:1995,completed:[],collections:[],visitedScenes:[choice.scene],visitedYears:[1995],score:0,settings});completionLocks.current.clear();setProfile(next);setSceneId(choice.scene);persistProfile(next)}
 ...
 function completeTrigger(item){const already=completionLocks.current.has(item.id);if(!already)completionLocks.current.add(item.id);const reward=collections[(Math.max(1,item.no)-1)%collections.length];setProfile(current=>{if(!current)return current;const currentCompleted=current.completed||[];if(currentCompleted.includes(item.id))return current;const currentCollections=current.collections||[];return {...current,completed:uniq([...currentCompleted,item.id]),collections:currentCollections.includes(reward.id)?currentCollections:uniq([...currentCollections,reward.id]),score:(current.score||0)+item.points}});showToast(already?'Memori diulang · progress tidak dihitung dua kali':`Memori tersimpan · +${item.points} · ${reward.label}`);audio.cue(item.mechanic==='arcade'?'arcade':item.mechanic==='phone'?'phone':item.mechanic==='dialup'?'dialup':'default')}
 ...
 if(overlay==='archive')return <ArchivePortal entry={archiveEntry} total={catalog.entries.length} onClose={()=>{setOverlay(null);setArchiveEntry(null)}}/>;
 ...
 return <div className={`world-app intensity-${settings.intensity}`} style={{'--year-accent':yearWorldState[year].accent}}>
  ...
  {overlay==='season'&&<SeasonPanel active={specialMode} onSelect={mode=>{toggleSpecialMode(mode);setOverlay(null)}} onClose={()=>setOverlay(null)}/>} 
  {overlay==='settings'&&<SettingsPanel settings={settings} onChange={next=>{const safe=sanitizeWorldSettings(next);setSettings(safe);setProfile(current=>current?{...current,settings:safe}:current)}} onClose={()=>setOverlay(null)} onCachePack={cacheMemoryPack} onClearPacks={clearMemoryPacks} offlineStatus={offlineStatus} onReset={resetJourney} memoryPacks={memoryPacks} isOnline={isOnline}/>} 
  ...
  {event&&!overlay&&!activeObject&&<DialogSurface className="ambient-event" label={event.seasonal?'Memori musiman':'Kejadian random'} onClose={()=>setEvent(null)}><small>{event.seasonal?'SEASONAL MEMORY':'KEJADIAN RANDOM'}</small><b>{event.label}</b><button data-autofocus onClick={()=>{const item=getTrigger(event.trigger);setEvent(null);if(item)launchTrigger(item)}}>Ikuti ↘</button><button onClick={()=>setEvent(null)}>Lewati</button></DialogSurface>}
 </div>;
}